...
def home():
    ...
    if ip_address =='': #ip_address is undefined, likely missing fall-back environment variable
        return render_template('error.html', error_status='Unable to get your IP address.')
    ...
    if country != 'US':
        return render_template('error.html', error_status=f'No data for country {country}.')
    ...
    return render_template('index.html', zipcode=zipcode, county=county_name, population=county_population, density=density, lat=lat, lng=lng)
...
def address_query():
    ...
    # API didn't return what was expected
        ...
        return render_template('error.html', error_status=f'Address {address_q} is not valid.')
    # catch other network issues 
    except requests.exceptions.RequestException:
        return render_template('error.html', error_status='Unable to reach the geolocation API server. Try again later.')
    #  get census data using geo coordinates
    if lat == "":
        return render_template('error.html', error_status=f'Address {address_q} is not valid.')
    try:
        ...
        return render_template('error.html', error_status='Unable to reach the census data API.')
    # No errors, success path
    return render_template('index.html', address=address_q, county=county_name, population=county_population, density=density, lat=lat, lng=lng)
